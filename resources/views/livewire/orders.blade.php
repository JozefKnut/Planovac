    ...
    <div class="bg-white rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.07)] p-4 sm:p-6 mb-5">
        ...
        <div class="mb-4">
            ...
            <input wire:model="zakaznik" type="text" placeholder="Meno zákazníka"
                class="border border-gray-200 rounded-lg px-3.5 py-2.5 text-[0.95rem] outline-none w-full sm:w-[340px]" />
        ...
        @foreach($polozky as $i => $polozka)
            <div class="flex items-center gap-2 mb-2.5 flex-wrap">
                ...
                <select wire:model.live="polozky.{{ $i }}.vyrobok_id"
                    class="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none flex-1 min-w-[160px]">
                ...
                <input wire:model.live="polozky.{{ $i }}.mnozstvo" type="number" placeholder="Množstvo"
                    min="0.5" step="0.5"
                    class="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none w-[100px]" />
        ...
        <div class="flex flex-col sm:flex-row justify-end items-start sm:items-center gap-3 sm:gap-5 pt-4 border-t border-gray-100">
            ...
            <button wire:click="ulozitZakazku"
                class="bg-violet-700 text-white border-0 rounded-lg px-6 py-3 text-[0.95rem] font-semibold cursor-pointer w-full sm:w-auto">
                ✓ Uložiť zákazku
            </button>
        </div>
    </div>
                ...
                <tbody>
                    @forelse($nevybaveneZakazky as $z)
                        <tr class="border-b border-gray-100">
                            ...
                            <td class="px-3 py-3 font-semibold whitespace-nowrap">{{ number_format($z->celkom, 2) }} €</td>
                            <td class="px-3 py-3">
                                <span class="bg-amber-100 text-amber-600 rounded-full px-2.5 py-0.5 text-[0.72rem] font-bold uppercase tracking-wide whitespace-nowrap">Nevybavené</span>
                            </td>
                            <td class="px-3 py-3">
